...
func (s *SmartContract) PurchaseData(ctx contractapi.TransactionContextInterface, sid string, buyerIDr string) (string, error) {

	buyerID := strings.Split(buyerIDr, ",")
	subscriptionID := generateHash(sid + buyerIDr)
...
func (s *SmartContract) BuyerAuthentication(ctx contractapi.TransactionContextInterface, subid string, buyerID []string, token string) (bool, error) {
	...
	if subcription.BuyerID[0] == buyerID[0] && subcription.DownloadToken == token {
		return true, nil
	} else {
