...
func ClientUpload(clients int, messagesize, messagecount int, filename string) {
	...
	reschBuy := make(chan *RunResults)
	enctext := make(chan string)
	ctresults := make([]string, clients)
	start := time.Now()
	for i := 0; i < clients; i++ {
		go runUpload(i+20, resch, enctext, messagesize, messagecount)
		ctresults[i] = <-enctext
	}

	for i := 0; i < clients; i++ {
		go rundownload(i+20, reschBuy, ctresults[i], messagesize, messagecount)
	}

	results := make([]*RunResults, clients*2)
	for i := 0; i < clients; i++ {
		results[i] = <-resch
	}

	for i := clients; i < clients*2; i++ {
		results[i] = <-reschBuy
	}

	totalTime := time.Now().Sub(start) //计算总运行时间
